...
@jit(nopython=True, cache=True)
def calc_bift_errors(opt_params, q, i, err, N, mc_runs=300,
    single_proc=False):
...
def doBift(q, i, err, filename, npts, alpha_min, alpha_max, alpha_n, dmax_min,
    dmax_max, dmax_n, mc_runs, queue=None,
    single_proc=False):
    ...
    if opt_res.get('success'):
        ...
        err_calc = calc_bift_errors((alpha, dmax), q, i, err, N, mc_runs,single_proc=single_proc)
        ...
        iftm = np.array(pr, r, err_interp, i, q, err, fit, results, fit_extrap, q_extrap)
...
data0=np.loadtxt('/Volumes/SSD_Alpha/Research/Documents/Projects/RubisCO/Thermococcus_Kodakarensis/SAXS/S_A_tkRubisco0mpa_1_data_000001.dat',
    dtype={'names': ('Q', 'I(Q)','ERROR'), 'formats': (np.float,np.float,np.float)},skiprows=4)

q=numba.float64[data0['Q']]
i=numba.float64[data0['I(Q)']]
err=numba.float64[data0['ERROR']]


doBift(q,i,err,'S_A_tkRubisco0mpa_1_data_000001',alpha_min=0.5,alpha_max=1,alpha_n=3,dmax_min=100,dmax_max=330,npts=50,dmax_n=5,mc_runs=2)
